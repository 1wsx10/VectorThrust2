...
public PIDContainer checkPID(string customData, float setp = -1, float seti = -1, float setd = -1) {
	...
	if(updateText) {
		...
		if(pLine == -1) {
			pLine = markerLine + 1;
			linesList.Insert(pLine, $"P: {p_val}");
			reset_p = false;
			fixPCaps = false;
		}
		if(iLine == -1) {
			iLine = pLine + 1;
			linesList.Insert(iLine, $"I: {i_val}");
			reset_i = false;
			fixICaps = false;
		}
		if(dLine == -1) {
			dLine = iLine + 1;
			linesList.Insert(dLine, $"D: {d_val}");
			reset_d = false;
			fixDCaps = false;
		}
		...
		// reset syntax errors
		if(reset_marker) {
			linesList[markerLine] = "PID";
		}
		if(reset_p) {
			linesList[pLine] = $"P: {p_val}";
			fixPCaps = false;
		}
		if(reset_i) {
			linesList[iLine] = $"I: {i_val}";
			fixICaps = false;
		}
		if(reset_d) {
			linesList[dLine] = $"D: {d_val}";
			fixDCaps = false;
		}
